...
def search_for_papers(query, result_limit=10) -> Union[None, List[Dict]]:
    """Search for papers using available APIs with priority order: SerpAPI -> Semantic Scholar"""
    
    # Try SerpAPI first if available
    serpapi_key = os.getenv("SERPAPI_KEY")
    if serpapi_key:
        try:
            from ai_scientist.tools.serpapi_scholar import search_for_papers_serpapi
            print("Using SerpAPI Google Scholar for search...")
            papers = search_for_papers_serpapi(query, result_limit)
            if papers:
                return papers
            print("SerpAPI search returned no results, falling back to Semantic Scholar...")
        except Exception as e:
            print(f"SerpAPI search failed: {e}, falling back to Semantic Scholar...")
    
    # Fallback to Semantic Scholar
    print("Using Semantic Scholar for search...")
...
def _search_semantic_scholar(query, result_limit=10) -> Union[None, List[Dict]]:
    """Internal function for Semantic Scholar search"""
    ...
    print(f"Semantic Scholar Response Status Code: {rsp.status_code}")
    print(
