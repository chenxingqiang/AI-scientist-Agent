...
from ai_scientist.tools.serpapi_scholar import SerpAPIScholarSearchTool
from ai_scientist.tools.base_tool import BaseTool
import os

# Create tool instances - prioritize SerpAPI if available
def get_search_tool():
    """Get the appropriate search tool based on available API keys"""
    if os.getenv("SERPAPI_KEY"):
        return SerpAPIScholarSearchTool()
    else:
...
ARGUMENTS:
<If ACTION is a search tool ("SearchSemanticScholar" or "SearchGoogleScholar"), provide the search query as {{"query": "your search query"}}. If ACTION is "FinalizeIdea", provide the idea details as {{"idea": {{ ... }}}} with the IDEA JSON specified below.>
